...
func compareVersionsForUpgrade(currentVersion *versionutil.Version, desiredVersion *versionutil.Version) error {
	if desiredVersion.LessThan(currentVersion) {
		return errors.New("current version is higher than desired version")
	}
	if desiredVersion.Major() - currentVersion.Major() != 0 || desiredVersion.Minor() - currentVersion.Minor() > 1 {
		return errors.New("desired version is too high to upgrade")
	}
	return nil
}
...
func (k *CheckUpgradeK8sVersion) Execute(_ connector.Runtime) error {
	...
	if desiredVersionObj.LessThan(maxVersionObj) {
		return errors.New("desired version to upgrade is less than the max version in cluster")
	}

	if err := compareVersionsForUpgrade(minVersionObj, desiredVersionObj); err != nil {
		return err
	}
